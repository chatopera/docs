...
如何大家做的变更公开了，但是却没有提交到春松客服 OSC 分支（春松客服源码的主分支），可能是缺少一份说明文档。这是本文档的作用：详细的说明如何提交代码到春松客服。
...
工作的内容记录在春松客服的工单(Issue)：https://github.com/chatopera/cskefu/issues

可以创建根据您自己的需要创建工单，或者选择一个已有的工单。

在工单中，对工作进行准确和清晰的定义，讨论，设计是非常有必要的，这个过程可以体现工作成熟度。

高手解决问题，在大脑中完成，动手实现只是完成确定无疑的事情。聪明的人，不断的完善头脑中创造的能力。多用手，少用脑会一直停留在菜鸟的阶段。

## 搭建开发环境

开发环境搭建，包括下载源码、安装 IDE、启动数据库、配置项目等。

详细内容，参考文档：https://docs.chatopera.com/products/cskefu/osc/engineering.html
...
Git 用于版本管理，使用 Git 时，一个文件有三个可能存在的地方：index，staged 和 workarea。indexed 文件就是被索引了（提交了）；staged 就是被临时的保存了（暂未提交，保护一下）；workarea，在编辑器中打开的这份。
