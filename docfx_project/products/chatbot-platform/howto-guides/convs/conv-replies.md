...
一个机器人的多轮对话，是由一个或多个话题组成，在机器人被创建的时刻，默认的初始化了 `greetings` 话题，这个话题可以回复【你好】。
...
第一行是**匹配器**，匹配器有多种形式：[通配符匹配器](https://docs.chatopera.com/products/chatbot-platform/references/gambit-star.html)、[模糊匹配器](https://docs.chatopera.com/products/chatbot-platform/howto-guides/convs/conv-gambit-like.html) 和 [意图匹配器](https://docs.chatopera.com/products/chatbot-platform/howto-guides/convs/conv-gambit-intent.html)。匹配器就是检查规则有没有被命中，如果命中就使用该规则的回复进行返回答案。

不同匹配器，有不同的语法。但是紧挨着匹配器的下一行是【回复】，回复必须以`- `开头，即【减号空格】。回复的用法很灵活，下面是具体的介绍。
